...
fs.mkdir('../fff/dd4').then(() =>{
    console.log('dd4 폴더 생성');
}).catch((err)=> {
    console.log('dd4 폴더 생성 에러') // 이미 존재하는 경우 생성할 수 없기 때문에 에러
})

fs.mkdir('../fff/dd5/ee5').then(() =>{
    console.log('dd5/ee5 폴더 생성');
}).catch((err)=> {
    console.log('dd5/ee5 폴더 생성 에러') // dd5가 존재하지 않아 dd5를 생성 불가
})

// recursive :true 를 이용 dd6생성 이후 ee6 생성;
...
// recursive :true 를 이용 dd6생성 이후 ee6 생성;
fs.mkdir('../fff/dd6/ee6',{recursive:true}).then(() =>{
    console.log('dd5/ee6 폴더 생성');
}).catch((err)=> {
    console.log('dd6/ee6 폴더 생성 에러');
});

fs.rename('../fff/dd6', '../qqq/aa6').then(()=>{
    console.log('dd5->aa6 이름 바꾸기(이름) 성공')

})catch((err)=>{
    console.log('dd6 -> aa6 이름바꾸기(이동) 에러');
});
...
fs.rmdir('../fff/dd1').then(()=>{

}).catch((err)=>.then(()=> {
    console.log('dd71폴더 삭제 성공');
}).catch((err))=>{
    console.log('dd 폴더 삭제 에러');
});

fs.copyFile('../fff/dd1','../qqq/aa1').then(()=>{
